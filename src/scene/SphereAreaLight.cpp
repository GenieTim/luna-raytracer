...
SphereAreaLight::SphereAreaLight(const Transform &tr, const double intensity,
                                 const Color &color, const double radius,
                                 const bool show, const Attenuation attenuation)
    : Light(tr, intensity, color), LightHasPosition(tr(Point(0, 0, 0))),
      LightHasAttenuation(attenuation), LightHasGeometry(show),
      m_radius(radius), m_minSamples(16), m_maxSamples(32) {
...
bool SphereAreaLight::computeSample(const Point &P, const Vector &N,
                                    ...
                                    unsigned int &haltonSeq, Color &I,
                                    Vector &L) const {
...
Geometry *SphereAreaLight::getGeometry() const { return new Sphere(m_radius); }

Material *SphereAreaLight::getMaterial() const {
  Material *m =
      new Material(std::string("_sal_") + std::to_string(m_id), getColor());
